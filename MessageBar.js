...
import { Text, View, TouchableOpacity, Animated, Dimensions, Image } from "react-native";
...
class MessageBar extends Component {
	...
	getStateByProps(props) {
		...
		return {
			...
			shouldHideOnTap: props.shouldHideOnTap == undefined && def.shouldHideOnTap == undefined ? true : props.shouldHideOnTap || def.shouldHideOnTap,
			...
			titleNumberOfLines: props.titleNumberOfLines == undefined && def.titleNumberOfLines == undefined ? 1 : props.titleNumberOfLines || def.titleNumberOfLines,
			messageNumberOfLines: props.messageNumberOfLines == undefined && def.titleNumberOfLines == undefined ? 2 : props.messageNumberOfLines || def.messageNumberOfLines,
	...
	renderTitle() {
		if (this.state.title != null) {
			return (
				<Text numberOfLines={this.state.titleNumberOfLines} style={[this.state.titleStyle, { color: this.state.titleColor }]}>
					{this.state.title}
				</Text>
			);
		}
	...
	renderMessage() {
		...
		if (this.state.message != null) {
			controls.push(
				<Text key="message" numberOfLines={this.state.messageNumberOfLines} style={[this.state.messageStyle, { color: this.state.messageColor }]}>
					{this.state.message}
				</Text>
			);
		}
		if (this.state.children != null) {
			controls.push(this.state.children);
		}
		return controls;
	}
}
